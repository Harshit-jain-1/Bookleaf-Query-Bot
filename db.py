 ...
 "cells": [
  {
   ...
   "source": [
    ...
    "intent, confidence (0-100), requires_db (True/False)\n",
    "Example:\n",
    "{'intent': 'book_live_status', 'confidence': 90, 'requires_db': True}\n",
    "\"\"\"\n",
    "\n",
    ...
    "            temperature=0\n",
    "        )\n",
    "        # Directly evaluate response as Python dictionary\n",
    "        content = resp.choices[0].message.content.strip()\n",
    "        # Ensure string is a valid dict\n",
    "        if content.startswith(\"{\") and content.endswith(\"}\"):\n",
    "            # Convert string to dict safely\n",
    "            return eval(content)  # WARNING: eval only safe if you control LLM output\n",
    "        else:\n",
    ...
    "    elif \"royalty\" in query_lower:\n",
    "        return {'intent': 'royalty_status', 'confidence': 90, 'requires_db': True}\n",
    "    else:\n",
    "        return {'intent': 'general_query', 'confidence': 75, 'requires_db': False}\n"
   ]
  },
  {
   "cell_type": "code",
